...
test("rejects YouTube metadata beyond platform limits",()=>{const item=rendered();item.publish={status:"ready",title:"x".repeat(101),description:"ok",platforms:["youtube"]};let result=validatePublishItem(item);assert.equal(result.valid,false);assert.ok(result.issues.some((issue)=>issue.field==="title"));item.publish={status:"ready",title:"ok",description:"x".repeat(5001),platforms:["youtube"]};result=validatePublishItem(item);assert.equal(result.valid,false);assert.ok(result.issues.some((issue)=>issue.field==="description"));});
