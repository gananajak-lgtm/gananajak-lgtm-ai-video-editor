...
test("rejects YouTube metadata beyond platform limits",()=>{const candidate=item();candidate.publish={status:"ready",title:"x".repeat(101),description:"ok",platforms:["youtube"]};let result=validatePublishItem(candidate);assert.equal(result.valid,false);assert.ok(result.issues.some((issue)=>issue.field==="title"));candidate.publish={status:"ready",title:"ok",description:"x".repeat(5001),platforms:["youtube"]};result=validatePublishItem(candidate);assert.equal(result.valid,false);assert.ok(result.issues.some((issue)=>issue.field==="description"));});

test("does not apply YouTube title length to other platforms",()=>{const result=validatePublishItem(item({publish:{status:"ready",title:"x".repeat(101),platforms:["instagram"]}}));assert.equal(result.issues.some((issue)=>issue.field==="title" && issue.platform==="youtube"),false);});
